...
public class PinnedSectionListView extends ListView {
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			...
			if (isFirstVisibleItemSection) {
				...
				if (sectionView.getTop() == getPaddingTop()) {
					...
					if (mPinnedSection != null && firstVisibleItem == mPinnedSection.position && sectionView.getTop() < 0) {
	...
	public void setFloatItemHeight(int floatItemHeight) {
		this.floatItemHeight = floatItemHeight;
	}
	...
	void createPinnedShadow(int position) {
		...
		if (layoutParams == null){
			if(floatItemHeight==0){
				floatItemHeight=getChildAt(0).getHeight();
			}
			
			layoutParams = (AbsListView.LayoutParams) new AbsListView.LayoutParams(AbsListView.LayoutParams.MATCH_PARENT,
					AbsListView.LayoutParams.WRAP_CONTENT, 0);
			if (layoutParams.height != floatItemHeight) {
				layoutParams.height = floatItemHeight;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mPinnedSection != null) {
			recreatePinnedLayout(l, t, r, b);
		}
	}
	
	private void recreatePinnedLayout(int l, int t, int r, int b){
	...
	private void clipRectDispatchDraw(Canvas canvas){
	...
	private void touchTargetInit(MotionEvent ev){
		final float x = ev.getX();
		final float y = ev.getY();

		if (ev.getAction() == MotionEvent.ACTION_DOWN && mTouchTarget == null && mPinnedSection != null
	...
	private void touchTargetDispatch(MotionEvent ev){
		final float x = ev.getX();
		final float y = ev.getY();
